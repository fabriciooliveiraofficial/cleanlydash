...
export function DialerWidget() {
  ...
  // Ativar Live Coach durante chamadas ativas
  const { currentTip } = useLiveCoach(callState === 'active')

  // Auto-abrir se estiver tocando
  React.useEffect(() => {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className={cn(
          "fixed bottom-6 right-6 h-14 w-14 rounded-full bg-indigo-600 text-white shadow-[0_10px_30px_rgba(99,102,241,0.4)] flex items-center justify-center transition-all hover:scale-110 active:scale-95 z-50 hover:shadow-[0_10px_40px_rgba(99,102,241,0.6)] backdrop-blur-sm border border-white/20",
  ...
  return (
    <div className="glass-panel fixed bottom-6 right-6 w-80 rounded-[32px] overflow-hidden flex flex-col z-50 animate-in slide-in-from-bottom-4 duration-500 border-white/60 shadow-[0_30px_60px_rgba(0,0,0,0.12)]">
      {/* Header Glass */}
      <div className="p-4 border-b border-white/20 bg-white/10 backdrop-blur-md flex items-center justify-between">
